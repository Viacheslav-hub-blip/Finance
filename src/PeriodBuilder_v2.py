...
class PeriodBuilder():
    ...
    def all_decline_periods(self, data: dict) -> []:
        dates, prices = list(data.keys()), list(data.values())
        left_point = 0
        all_periods = []
        for right_point in range(len(data) - 1):
            current_price = round(prices[right_point], 3)
            next_price = round(prices[right_point + 1], 3)

            if current_price > next_price:
                pass
            else:
                include_prices = prices[left_point:right_point + 1]
                include_dates = dates[left_point:right_point + 1]
                left_point = right_point + 1
                current_period = {}
                for date, price in zip(include_dates, include_prices):
                    current_period[date] = price

                all_periods.append(current_period)

        return all_periods

    def all_rise_period(self, data: dict) -> []:
        dates, prices = list(data.keys()), list(data.values())
        left_point = 0
        all_periods = []
        for right_point in range(len(data) - 1):
    ...
    def choose_periods_by_length_and_change(self, all_periods: [{}], period_len: int, change_price: float) -> [{}]:
        all_periods = [period for period in all_periods if len(period) >= period_len]
        new_all_periods = []
        for period in all_periods:
            change = self._get_change_price(list(period.values())[0], list(period.values())[-1])
            if abs(change) >= change_price:
                new_all_periods.append(period)
        return new_all_periods

    def _get_change_price(self, start_value: float, last_values: int) -> float:
        return round(((last_values - start_value) / start_value) * 100, 3)

    def create_Periods_list(self, company: str, all_periods: [{}]) -> [Period]:
        periods = [Period]
        for period in all_periods:
            first_date, last_date = list(period.keys())[0], list(period.keys())[-1]
            first_price, last_price = list(period.values())[0], list(period.values())[-1]
            change = self._get_change_price(first_price, last_price)
            # count_days = abs((datetime.datetime.strptime(first_date, '%Y-%m-%d') - datetime.datetime.strptime(last_date,
            #                                                                                                   '%Y-%m-%d')).days) + 1
            periods.append(Period(company, first_date, last_date, first_price, last_price, change))
...
if __name__ == "__main__":
    ...
    all_periods = period_builder.all_rise_period(ar_data)
    choosen_periods = period_builder.choose_periods_by_length_and_change(all_periods, 3, 6)
    periods_list = period_builder.create_Periods_list('APPL', choosen_periods)
